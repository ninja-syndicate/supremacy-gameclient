...
// Copied from Unreal source and modified for the purpose of writing PascalCase JSON.

/** Class that handles converting Json objects to and from UStructs */
class SUPREMACY_API FPascalCaseJsonObjectConverter
{
public:
...
public: // JSON -> UStruct
	...
	static bool JsonObjectStringToUStruct(const FString& JsonString, OutStructType* OutStruct, int64 CheckFlags = 0, int64 SkipFlags = 0, const bool bStrictMode = false)
	{
		...
		if (!FPascalCaseJsonObjectConverter::JsonObjectToUStruct(JsonObject.ToSharedRef(), OutStruct, CheckFlags, SkipFlags, bStrictMode))
		{
	...
	*/
	...
	static bool JsonArrayToUStruct(const TArray<TSharedPtr<FJsonValue>>& JsonArray, TArray<OutStructType>* OutStructArray, int64 CheckFlags = 0, int64 SkipFlags = 0, const bool bStrictMode = false)
	{
		...
		for (int32 i = 0; i < JsonArray.Num(); ++i)
		{
			...
			if (!FPascalCaseJsonObjectConverter::JsonObjectToUStruct(Value->AsObject().ToSharedRef(), OutStructType::StaticStruct(), &(*OutStructArray)[i], CheckFlags, SkipFlags, bStrictMode))
			{
