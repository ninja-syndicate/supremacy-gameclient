...
FString FPascalCaseJsonObjectConverter::StandardizeCase(const FString& StringIn)
{
...
namespace
{
	...
	TSharedPtr<FJsonValue> ConvertScalarFPropertyToJsonValue(FProperty* Property, const void* Value, int64 CheckFlags, int64 SkipFlags, const FPascalCaseJsonObjectConverter::CustomExportCallback* ExportCb, FProperty* OuterProperty)
	{
		...
		else if (FArrayProperty* ArrayProperty = CastField<FArrayProperty>(Property))
		{
			...
			{
				TSharedPtr<FJsonValue> Elem = FPascalCaseJsonObjectConverter::UPropertyToJsonValue(ArrayProperty->Inner, Helper.GetRawPtr(i), CheckFlags & (~CPF_ParmFlags), SkipFlags, ExportCb, ArrayProperty);
				if (Elem.IsValid())
				{
					// add to the array
					Out.Push(Elem);
		...
		else if (FSetProperty* SetProperty = CastField<FSetProperty>(Property))
		{
			...
			for (int32 i = 0, n = Helper.Num(); n; ++i)
			{
				...
				{
					TSharedPtr<FJsonValue> Elem = FPascalCaseJsonObjectConverter::UPropertyToJsonValue(SetProperty->ElementProp, Helper.GetElementPtr(i), CheckFlags & (~CPF_ParmFlags), SkipFlags, ExportCb, SetProperty);
					if (Elem.IsValid())
					{
						// add to the array
						Out.Push(Elem);
		...
		else if (FMapProperty* MapProperty = CastField<FMapProperty>(Property))
		{
			...
			for (int32 i = 0, n = Helper.Num(); n; ++i)
			{
				if (Helper.IsValidIndex(i))
				{
					TSharedPtr<FJsonValue> KeyElement = FPascalCaseJsonObjectConverter::UPropertyToJsonValue(MapProperty->KeyProp, Helper.GetKeyPtr(i), CheckFlags & (~CPF_ParmFlags), SkipFlags, ExportCb, MapProperty);
					TSharedPtr<FJsonValue> ValueElement = FPascalCaseJsonObjectConverter::UPropertyToJsonValue(MapProperty->ValueProp, Helper.GetValuePtr(i), CheckFlags & (~CPF_ParmFlags), SkipFlags, ExportCb, MapProperty);
					if (KeyElement.IsValid() && ValueElement.IsValid())
					{
						...
						{
							KeyString = FPascalCaseJsonObjectConverter::StandardizeCase(KeyString);
						}
		...
		else if (FStructProperty* StructProperty = CastField<FStructProperty>(Property))
		{
			...
			TSharedRef<FJsonObject> Out = MakeShared<FJsonObject>();
			if (FPascalCaseJsonObjectConverter::UStructToJsonObject(StructProperty->Struct, Value, Out, CheckFlags & (~CPF_ParmFlags), SkipFlags, ExportCb))
			{
				return MakeShared<FJsonValueObject>(Out);
			}
		}
		else if (FObjectProperty* ObjectProperty = CastField<FObjectProperty>(Property))
		{
			...
			if (Object && (ObjectProperty->HasAnyPropertyFlags(CPF_PersistentInstance) || (OuterProperty && OuterProperty->HasAnyPropertyFlags(CPF_PersistentInstance))))
			{
				...
				if (FPascalCaseJsonObjectConverter::UStructToJsonObject(ObjectProperty->GetObjectPropertyValue(Value)->GetClass(), Object, Out, CheckFlags, SkipFlags, ExportCb))
				{
...
TSharedPtr<FJsonValue> FPascalCaseJsonObjectConverter::ObjectJsonCallback(FProperty* Property, const void* Value)
{
	if (FObjectProperty* ObjectProperty = CastField<FObjectProperty>(Property))
	{
		if (!ObjectProperty->HasAnyFlags(RF_Transient)) // We are taking Transient to mean we don't want to serialize to Json either (could make a new flag if nessasary)
		{
			...
			CustomCB.BindStatic(FPascalCaseJsonObjectConverter::ObjectJsonCallback);

			void** PtrToValuePtr = (void**)Value;

			if (FPascalCaseJsonObjectConverter::UStructToJsonObject(ObjectProperty->PropertyClass, (*PtrToValuePtr), Out, 0, 0, &CustomCB)) //-V549
			{
				return MakeShared<FJsonValueObject>(Out);
			}
		}
...
TSharedPtr<FJsonValue> FPascalCaseJsonObjectConverter::UPropertyToJsonValue(FProperty* Property, const void* Value, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb, FProperty* OuterProperty)
{
...
bool FPascalCaseJsonObjectConverter::UStructToJsonObject(const UStruct* StructDefinition, const void* Struct, TSharedRef<FJsonObject> OutJsonObject, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
	return UStructToJsonAttributes(StructDefinition, Struct, OutJsonObject->Values, CheckFlags, SkipFlags, ExportCb);
}

bool FPascalCaseJsonObjectConverter::UStructToJsonAttributes(const UStruct* StructDefinition, const void* Struct, TMap< FString, TSharedPtr<FJsonValue> >& OutJsonAttributes, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
	if (SkipFlags == 0)
	{
...
bool FPascalCaseJsonObjectConverter::UStructToJsonObjectString(const UStruct* StructDefinition, const void* Struct, FString& OutJsonString, int64 CheckFlags, int64 SkipFlags, int32 Indent, const CustomExportCallback* ExportCb, bool bPrettyPrint)
{
...
//static
bool FPascalCaseJsonObjectConverter::GetTextFromObject(const TSharedRef<FJsonObject>& Obj, FText& TextOut)
{
	...
	bool ConvertScalarJsonValueToFPropertyWithContainer(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* OutValue, const UStruct* ContainerStruct, void* Container, int64 CheckFlags, int64 SkipFlags, const bool bStrictMode)
	{
		...
		else if (FTextProperty* TextProperty = CastField<FTextProperty>(Property))
		{
			...
			else if (JsonValue->Type == EJson::Object)
			{
				...
				FText Text;
				if (!FPascalCaseJsonObjectConverter::GetTextFromObject(Obj.ToSharedRef(), Text))
				{
...
bool FPascalCaseJsonObjectConverter::JsonValueToUProperty(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* OutValue, int64 CheckFlags, int64 SkipFlags, const bool bStrictMode)
{
	return JsonValueToFPropertyWithContainer(JsonValue, Property, OutValue, nullptr, nullptr, CheckFlags, SkipFlags, bStrictMode);
}

bool FPascalCaseJsonObjectConverter::JsonObjectToUStruct(const TSharedRef<FJsonObject>& JsonObject, const UStruct* StructDefinition, void* OutStruct, int64 CheckFlags, int64 SkipFlags, const bool bStrictMode)
{
	return JsonAttributesToUStruct(JsonObject->Values, StructDefinition, OutStruct, CheckFlags, SkipFlags, bStrictMode);
}

bool FPascalCaseJsonObjectConverter::JsonAttributesToUStruct(const TMap< FString, TSharedPtr<FJsonValue> >& JsonAttributes, const UStruct* StructDefinition, void* OutStruct, int64 CheckFlags, int64 SkipFlags, const bool bStrictMode)
{
...
//static 
bool FPascalCaseJsonObjectConverter::GetTextFromField(const FString& FieldName, const TSharedPtr<FJsonValue>& FieldValue, FText& TextOut)
{
	if (FieldValue.IsValid())
	{
		...
		{
			// localized string
			if (FPascalCaseJsonObjectConverter::GetTextFromObject(FieldValue->AsObject().ToSharedRef(), TextOut))
			{
				return true;
			}
...
FFormatNamedArguments FPascalCaseJsonObjectConverter::ParseTextArgumentsFromJson(const TSharedPtr<const FJsonObject>& JsonObject)
{
